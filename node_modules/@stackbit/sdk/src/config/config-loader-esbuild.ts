...
export type StopConfigWatch = { stop?: () => void };
    ...
    return async (error: esbuild.BuildFailure | null, result: esbuild.BuildResult | null) => {
        ...
        if (error) {
            ...
            try {
                ...
                logger?.debug('error in load stackbit config callback');
            }
        }
    };
}

async function getConfigFromResult(result: esbuild.BuildResult, fileName: string, projectDir: string): Promise<LoadStackbitConfigResult> {
    try {
