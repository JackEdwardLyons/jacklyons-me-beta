...
function extendModel(model, modelName, modelsByName, _extendPath = []) {
    ...
    lodash_1.default.forEach(_extends, (superModelName) => {
        ...
        utils_1.copyIfNotSet(extendedSuperModel, 'hideContent', model, 'hideContent');
        utils_1.copyIfNotSet(extendedSuperModel, 'singleInstance', model, 'singleInstance');
        utils_1.copyIfNotSet(extendedSuperModel, 'labelField', model, 'labelField');
        utils_1.copyIfNotSet(extendedSuperModel, 'variantField', model, 'variantField');
        if (Array.isArray(extendedSuperModel.fieldGroups) && extendedSuperModel.fieldGroups.length > 0) {
            model.fieldGroups = lodash_1.default.uniqBy(lodash_1.default.concat(extendedSuperModel.fieldGroups, lodash_1.default.get(model, 'fieldGroups', [])), 'name');
        }
        let idx = 0;
