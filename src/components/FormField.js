  ...
  render() {
    ...
    return _.get(field, "input_type", null) === "checkbox" ? (
      <div className="form-group form-checkbox">
        ...
        />
        {_.get(field, "label", null) && <label htmlFor={_.get(field, "name", null)}>{_.get(field, "label", null)}</label>}
      </div>
      ...
      <div className="form-group">
        ...
        <div className="form-select-wrap">
          <select id={_.get(field, "name", null)} name={_.get(field, "name", null)} {...(_.get(field, "is_required", null) ? { required: true } : null)}>
            {_.get(field, "default_value", null) && <option value="">{_.get(field, "default_value", null)}</option>}
      ...
      <div className="form-group">
        ...
        <textarea
          ...
          id={_.get(field, "name", null)}
          rows="5"
          {...(_.get(field, "default_value", null) ? { placeholder: _.get(field, "default_value", null) } : null)}
          {...(_.get(field, "is_required", null) ? { required: true } : null)}
        />
